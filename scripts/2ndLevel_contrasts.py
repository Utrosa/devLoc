...
save_fig       = True
save_average   = True  # Applies to the summed contrast arrays
average_voxels = False # Do we average voxels per ROI or not? 
...
for idx, (_, row) in enumerate(df_plot.iterrows()):
    ...
    ax.scatter(0.03, mean_val, 
               color="#F7F3EF", edgecolor='#F7F3EF', s=25, marker='o', 
               zorder=11, linewidth=0.5, label='Mean')
    
    # Plot Error Bar (SEM)
    ax.errorbar(0.03, mean_val, yerr=sem_val, 
                fmt='none', ecolor='#FF8C00', linewidth=1.2, 
                capsize=5, capthick=2, zorder=12)
...
fig, axes = plt.subplots(rows, cols, figsize=(6.5 * cols, 5 * rows), constrained_layout=True)
axes = axes.flatten()
...
for idx, (_, row) in enumerate(df_plot.iterrows()):
    ...
    if is_sig:
        ...
        ax.text(0.5, text_y, p_str, ha='center', va='top', fontsize=10, color='black', fontweight='bold')
    else:
        ax.text(0.5, bracket_y, 'ns', ha='center', va='bottom', fontsize=12, color='gray', style='italic')

    # E. Formatting
    ax.set_title(f"d = {row['cohen_d']:.2f}", fontsize=11, pad=12, fontweight='bold')
    ax.set_xlabel("")
    ax.set_ylabel("")
    ax.set_xticks([0, 1])
    ax.set_xticklabels([roi_a_name, roi_b_name], fontsize=10, weight='bold')
