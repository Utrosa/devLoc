function tapas(homePath, subID, sesID, project, task, acqIDXs)
%% Preparation
    ...
    fprintf('%d FUNCTIONAL SEQUENCES FOUND in THE MRI DIRECTORY:\n%s\n', numel(bids_names), strjoin(bids_names, ',\n'));
...
%% APPLY TAPAS TO THE FUNCTIONAL SEQUENCE/S -------------------------------
    ...
    display(acqIDXs);
    for idx = 1:length(acqIDXs) % Index acquisitions IDs in alphabetical order
        i = acqIDXs(idx);
        fprintf('\nAPPLYING TAPAS TO:\n%s\n', bids_names{i});
        
        %% Create main input structure - PhysIO
        ...
        physio.log_files.vendor = 'Biopac_Mat'; % 'Biopac_Mat' or 'Biopac_Txt'
